...
**What does what?**
* **`java_version/`**: Java backend that reads `.pcap` files, parses network protocols byte-by-byte, extracts domains, and exports a `report.json` file containing the analysis.
* **`dashboard/`**: UI built with React. It simply reads the `report.json` file generated by the backend and displays it using interactive charts.
